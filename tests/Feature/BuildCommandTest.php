...
it('fails with deadlock when all pending tasks have circular dependencies', function () {
    $tasksPath = $this->testPath.'/.laracode/specs/test-feature/tasks.json';
    file_put_contents($tasksPath, json_encode([
        'title' => 'Deadlock Feature',
        'tasks' => [
            ['id' => 1, 'title' => 'Task A', 'status' => 'pending', 'dependencies' => [2]],
            ['id' => 2, 'title' => 'Task B', 'status' => 'pending', 'dependencies' => [3]],
    ...
    $this->artisan('build', ['path' => $tasksPath])
        ->assertFailed()
        ->expectsOutputToContain('Deadlock');
});

it('fails with deadlock when pending tasks depend on non-existent tasks', function () {
    $tasksPath = $this->testPath.'/.laracode/specs/test-feature/tasks.json';
    file_put_contents($tasksPath, json_encode([
        'title' => 'Missing Dep Feature',
        'tasks' => [
            ['id' => 1, 'title' => 'Task 1', 'status' => 'pending', 'dependencies' => [99]],
        ],
    ]));
...
it('deregisters session on deadlock', function () {
    $registryPath = $this->testPath.'/.laracode/sessions.json';
    $tasksPath = $this->testPath.'/.laracode/specs/test-feature/tasks.json';
    file_put_contents($tasksPath, json_encode([
        'title' => 'Deadlock Session Test',
        'tasks' => [
            ['id' => 1, 'title' => 'Task A', 'status' => 'pending', 'dependencies' => [2]],
    ...
    expect($registry->getSessions())->toBeEmpty();
});
