...
describe('session rendering', function () {
    ...
    it('renders mode and agent on line 3', function () {
        ...
        expect($html)->toContain('Mode:')
            ->and($html)->toMatch('/text-cyan[^>]*>normal</')
            ->and($html)->toContain('Agent:')
            ->and($html)->toMatch('/text-cyan[^>]*>claude</');
    });
    ...
    it('renders progress and elapsed time on line 3', function () {
        ...
        expect($html)->toContain('Tasks:')
            ->and($html)->toMatch('/text-yellow[^>]*>2\/4/')
            ->and($html)->toContain('Status:')
            ->and($html)->toContain('Elapsed:')
            ->and($html)->toMatch('/text-yellow[^>]*>\d+s</');
    });
    ...
    it('renders status label active when task in_progress', function () {
        ...
        expect($html)->toMatch('/text-cyan[^>]*>active</');
    });
    ...
    it('renders status label complete when all done', function () {
        ...
        expect($html)->toMatch('/text-green[^>]*>complete</');
    });
    ...
    it('renders status label running when no tasks in_progress', function () {
        ...
        expect($html)->toMatch('/text-cyan[^>]*>running</');
    });
    ...
    it('renders branch in parentheses in cyan on line 2', function () {
        ...
        expect($html)->toMatch('/text-cyan[^>]*>\(feature\/xyz\)/');
    });
...
describe('registry status rendering', function () {
    ...
    it('renders red crashed label when PID is dead and status is running', function () {
        ...
        expect($html)->toMatch('/text-red[^>]*>crashed</');
    });
...
describe('elapsed time', function () {
    it('shows fixed elapsed for completed sessions', function () {
        $tasksPath = $this->tempDir.'/tasks.json';
        file_put_contents($tasksPath, json_encode([
            'title' => 'Done',
            'branch' => 'main',
            'tasks' => [['id' => 1, 'status' => 'completed', 'title' => 'A']],
        ]));
        ...
        $sessions = [
            [
                'tasksPath' => $tasksPath,
                'pid' => getmypid(),
                'startedAt' => date('c', time() - 300),
                'completedAt' => date('c', time() - 180),
                'mode' => 'normal',
                'agent' => 'claude',
                'projectPath' => '/project',
                'status' => 'completed',
        ...
        expect($html)->toContain('2m0s');
    });

    it('shows live elapsed for running sessions', function () {
        $tasksPath = $this->tempDir.'/tasks.json';
        file_put_contents($tasksPath, json_encode([
            'title' => 'Running',
            'branch' => 'main',
            'tasks' => [['id' => 1, 'status' => 'in_progress', 'title' => 'Working']],
        ]));
        ...
        expect($html)->toMatch('/text-yellow[^>]*>1[0-2]s</');
    });
});
...
describe('edge cases', function () {
    ...
    it('renders all completed progress', function () {
        ...
        expect($html)->toContain('Tasks:')
            ->and($html)->toMatch('/text-green[^>]*>2\/2/');
    });
});
