...
class BuildCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        while ($iteration < $maxIterations) {
            ...
            if ($nextTask === null) {
                if ($this->taskSelector->isDeadlocked($tasks['tasks'])) {
                    $blocked = $this->taskSelector->countBlockedTasks($tasks['tasks']);
                    $this->renderDashboard($tasks, $iteration, $maxIterations, $startTime, null, $mode,
                        "Deadlock: {$blocked} tasks blocked by unmet dependencies");
