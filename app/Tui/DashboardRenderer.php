...
class DashboardRenderer
{
    ...
    public function renderFinalStats(array $tasks): void
    {
        ...
        $html = <<<HTML
            <div>
                <div class="text-green-400 px-2 py-0 font-bold">Build Complete</div>
                <div class="my-1 px-2">
                    <div>Duration:      <span class="text-yellow-400">{$duration}</span></div>
                    <div>Files Changed: <span class="text-yellow-400">{$filesChanged}</span></div>
                    <div>Lines Added:   <span class="text-green-400">+{$linesAdded}</span></div>
                    <div>Lines Removed: <span class="text-red-400">-{$linesRemoved}</span></div>
                </div>
            </div>
        HTML;
