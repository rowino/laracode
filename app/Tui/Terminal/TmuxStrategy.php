 ...
 */
class TmuxStrategy implements TerminalStrategy, TerminalTabOpener
{
    ...
    {
        $escaped = escapeshellarg($cwd);
        $output = [];
        // Safe: $cwd is escaped via escapeshellarg
        exec("tmux new-window -c $escaped 2>/dev/null", $output, $exitCode);

        if ($exitCode !== 0) {
            return FocusResult::error('Failed to open new tmux window');
        }

        return new FocusResult(true, 'Opened new tmux window');
    }
