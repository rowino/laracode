    ...
    public function render(DashboardState $state): string
    {
        ...
        return <<<HTML
            <div class="flex justify-between text-white px-2 py-0">
                <span class="font-bold">laracode <span class="text-green">[Running]</span></span>
                <span class="text-gray">Iteration: <span class="text-cyan">{$state->currentIteration}/{$state->maxIterations}</span>  Time: <span class="text-yellow">{$time}</span></span>
            </div>
        HTML;
    }
}
